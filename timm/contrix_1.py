...
def save_accuracy(accuracy,yaml_name,K):
...
def contrix(args,yaml_name):
    ...
    print(yaml_name)
    yaml_name = yaml_name.split('.')[0]
    checkpoint = torch.load(f'./work_dirs2/{yaml_name}/model_50.pth')
    ...
    plot_confusion_matrix(c_m,obj_list,f'{yaml_name}')
    plt.savefig(f'1.6/{yaml_name}.png', format='png')
...
if __name__ == '__main__':
    ...
    yaml_names = os.listdir('config')
    yaml_names = ['resnet50_Harbor_Feature.yaml','resnet50_Ship_Feature.yaml']
    for yaml_name in yaml_names:
        ...
        parser.add_argument('-c', '--config', type=str, default=f'./config/{yaml_name}', help='config file')
        args = parser.parse_args()

        val_acc,K=contrix(args,yaml_name)
        show_accuracy(val_acc)
        save_accuracy(val_acc,yaml_name,K)
